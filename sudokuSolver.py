...
def solve(bo):
    find = find_empty(bo)
    if not find:
        return bo  # Return the solved board when no empty cell is found
    else:
        row, col = find
    for i in range(1, 10):
        if valid(bo, i, (row, col)):
            bo[row][col] = i
            if solve(bo):  # If solve returns a board, return it
                return bo
            bo[row][col] = 0
    return False  # If the board can't be solved, return False
